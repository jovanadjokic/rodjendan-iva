...
# CSS koji forsira dugme na sredinu ekrana (Viewport center)
st.markdown("""
    <style>
    .stApp {
    ...
    /* CSS magija: Postavlja dugme tačno na sredinu ekrana bez obzira na sve */
    div.stButton {
        display: flex;
        justify-content: center;
        align-items: center;
        height: 50vh; /* Zauzima polovinu visine ekrana da bi dugme bilo u centru */
    }
...
# 3. NASLOV (Ovo je uvek na vrhu)
st.markdown("<h1>🎉 Srećan rođendan, kraljice! 🎂</h1>", unsafe_allow_html=True)

# 4. KONTEJNER ZA DUGME ILI SADRŽAJ
glavni_ekran = st.empty()

if not st.session_state['prikazi_sadrzaj']:
    with glavni_ekran.container():
        # Dugme je unutar kontejnera koji nema ništa drugo ispod sebe
        if st.button('OTVORI IZNENAĐENJE 🎁'):
            st.session_state['prikazi_sadrzaj'] = True
            st.balloons()
            st.rerun()
else:
    # Kada se klikne, kontejner se prazni i iscrtava se ceo sajt
    with glavni_ekran.container():
        st.snow()
        ...
        with col_a:
            try: st.image('img.png', use_container_width=True)
            except: pass
        with col_b:
            try: st.image('img_2.png', use_container_width=True)
            except: pass
